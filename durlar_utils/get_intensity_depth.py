...
import os
import argparse
from bin_to_img import bin_to_img
import cv2
...
flags = read_args()
if flags.color:
    cNorm = colors.Normalize(vmin=0, vmax=max_range)
...
for bin_filepath in os.listdir(ouster_points_path):
    fullpath = os.path.join(ouster_points_path, bin_filepath)
    scan = (np.fromfile(fullpath, dtype=np.float32)).reshape(-1, 4)
    range_map, intensity = bin_to_img(scan=scan, rows=flags.rows, cols = flags.cols)
