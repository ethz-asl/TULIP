...
def main(args):
    ...
    # Get dataset
    dataset_train = build_multimae_pretraining_dataset(args, is_train=True)
    dataset_val = build_multimae_pretraining_dataset(args, is_train=False)
    ...
    if True:  # args.distributed:
        ...
        sampler_val = torch.utils.data.DistributedSampler(
            dataset_val, num_replicas=num_tasks, rank=sampler_rank, shuffle=False, drop_last=True,
        )
    ...
    if global_rank == 0 and args.log_wandb:
        os.environ["WANDB_CACHE_DIR"] = '/cluster/work/riner/users/biyang/wandb/.cache'
...
def evaluate(model: torch.nn.Module, data_loader: Iterable, tasks_loss_fn: Dict[str, torch.nn.Module],
                    loss_balancer: torch.nn.Module, device: torch.device, log_writer=None, start_steps=None,
                    num_encoded_tokens: int = 196, in_domains: List[str] = [], loss_on_unmasked: bool = True,
                    alphas: float = 1.0, sample_tasks_uniformly: bool = False, fp32_output_adapters: List[str] = []):
...
def get_masked_inputs(input_dict, masks, patch_size = (8, 8)):
    ...
    masks: (B, (H*W/(p*p))
    masked_input = {task: tensor}, tensor = (B, C, H, W)
    """
    masked_input = {}
    ph, pw = patch_size 
    for task, batch in input_dict.items():
        B, C, H, W = batch.shape
        h, w = int(H/ph), int(W/pw) 
        masked_imgs = batch.reshape(shape=(B, C, h, ph, w, pw))
