...
def test_pixel_based_network(output_path, pred_batch=1, h_high = 128, w_high = 2048, save_pcd = True, grid_size = 0.1):
    ...
    for packed_batches in tqdm(test_loader, leave=False, desc='test'):
        ...
        loss_low_res_part = loss_low_res_part.mean()* (test_dataset.lidar_out['norm_r'] / test_dataset.lidar_out['max_r'])

        mse_all = (np.absolute(pred - gt)).mean()* (test_dataset.lidar_out['norm_r'] / test_dataset.lidar_out['max_r'])


        # Already Denormalized in the previous step, we can set maximum range as 1 for denormalization in the prediction step
        pcd_pred = img_to_pcd_carla(pred, maximum_range=test_dataset.lidar_out['norm_r'])
        pcd_gt = img_to_pcd_carla(gt, maximum_range=test_dataset.lidar_out['norm_r'])
...
def test_implicit_network(output_path, pred_batch=1, h_high = 128, w_high = 2048, save_pcd = True, grid_size = 0.1):
    ...
    for packed_batches in tqdm(test_loader, leave=False, desc='test'):
        ...
        # print(iou, chamfer_dist, mse_all)
...
if __name__ == '__main__':
    ...
    dataset_config = {'name': 'Carla',
                      ...
                      'args': {'directory': config['dataset']['args']['directory'],
                               'scene_ids': ['Town07', 'Town10HD'],
                            #    'res_in': '16_1024',
                                 'res_in': config['dataset']['args']['res_in'],
                               'res_out': config['dataset']['args']['res_out'],
                               'memory_fetch': config['dataset']['args']['memory_fetch'],}}
